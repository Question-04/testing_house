...
const AccessoriesProductGrid: React.FC<AccessoriesProductGridProps & { loading?: boolean }> = ({ products, onProductClick, loading = false, mobile = false }) => {
  if (loading) {
  ...
  return (
    <div className={mobile ? styles.gridMobile : styles.grid}>
      {products.map(product => (
        <Link href={`/accessories/${product.id}`} key={product.id} legacyBehavior>
